...
export default {
  ...
  provider(text, record, index, column, provider) {
    const {provider: providerKey, textKey = 'label', valueKey = 'value', tipKey} = column;
  ...
  commaProvider(text, record, index, column, provider) {
    ...
    return typeof text === 'string' && text.split(',').map(itemValue => optionMap[itemValue] || itemValue);
  },
  combiner(text, record, index, column) {
    const {keys} = column;
    const content = keys.length >= 2 && `[${_.get(record, keys[0], '')}]${_.get(record, keys[1], '')}`
  ...
  objectArray(text, record, index, {itemTextKey = 'label', itemKeys}) {
    ...
    return Array.isArray(text) && text.map(item => item[itemTextKey]).join('、') || '';
  },
  ...
  thousandsSeparator(text) {
    ...
    if (typeof text === 'number') {
      return (`${text}`).replace(/(?=(?!^)(?:\d{3})+(?:\.|$))(\d{3}(\.\d+$)?)/g, ',$1');
    }
  ...
  link(text, record, index, column) {
    ...
    return <a href={`${finalLink}`} target={target}>{text}</a>
  }
};
