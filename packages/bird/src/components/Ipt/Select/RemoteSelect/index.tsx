...
export default class RemoteSelect extends React.Component {
  ...
  fetchOptions = (value, callback) => {
    ...
    const { api: { params, queryWrapKey='filter', ...otherApiOptions }, sidType = 'number', sidKey = 'id', stextKey='text' } = this.props;

    const searchParams = /\d+/.test(value) ? { [sidKey]: sidType === 'number' ? +value : `${value}` } : { [stextKey]: value };
    ...
    request({
      ...
      .then(response => {
        ...
        const data = response.data.list.map(item => ({
          text: Array.isArray(textKey) ? textKey.map(key => _.get(item, key)).join('-') : _.get(item, textKey),
    ...
    if (typeof value !== 'undefined') {
      this.fetchOptions(value, () => {
        ...
        if (data && data.length === 1) {
          ...
          this.setState({ value: { key: realValue, label: _.get(item, 'text') } }, () => {
            const { onChange } = this.props;
  ...
  render() {
    ...
    return (
      <Select
        ...
        notFoundContent={fetching ? <Spin size="small" /> : data.length === 0 && '没找到相关内容' }
        filterOption={false}
