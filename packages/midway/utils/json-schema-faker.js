...
function arrayType(value, path, resolve, traverseCallback) {
  ...
  if (optionalsProbability !== false) {
    if  (!maxItems) {
      maxItems = 5; // when optionalsProbability is not false, the maxItems value needed to avoid the empty array
    }
...
function objectType(value, path, resolve, traverseCallback) {
  ...
  if (optionalsProbability !== false) {
    if (fixedProbabilities === true) {
      ...
      console.log('needed:', random.number, max, min, neededExtras);
    }
  }
  ...
  console.log('fake:', neededExtras, random.shuffle, extraPropertiesRandomOrder, optionalProperties, extraProperties, _props);
...
function produce() {
  ...
  var separator = ' ';
  if (optionAPI('whitespace') === false) {
    separator = '_';
  }
  return wordsGenerator(length).join(separator);
}
/**
